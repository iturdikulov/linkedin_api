...
def get_random_base64(length=16):
    ...
    message_bytes = "".join((random.choice(letters_and_digits) for i in range(length))).encode(
        "ascii"
    )
...
def get_converstation_data(api, max_iterations, get_conversation_delay, log=None):
    ...
    for i in range(max_iterations):
        if i > 0:
            if log:
                log.debug(
                    "{0} sec delay before next get conversation...".format(get_conversation_delay)
                )
    ...
    if current_variables and isinstance(current_variables, dict):
        ...
        for string_variable in string_variables:
            ...
            if string_variable_index.isdigit() and string_variable_index in current_variables:
                invite_message = invite_message.replace(
                    string_variable, current_variables.get(string_variable_index)
                )
            else:
...
def get_conversations_additional_data(conversations_data, logger=None):
    ...
    conversations_users_participants = (
        {}
    )  # Users to whom only we wrote message and users which need sent follow up message
...
def parse_default_search_data(elements):
    ...
    for sub_element in elements:
        if (
            not users_order
            and sub_element.get("*results")
            and isinstance(sub_element.get("*results"), list)
        ):
...
def parse_search_hits(search_hits, is_sales=False, search_start=0):
    ...
    if not is_sales:
        ...
        if users_data:
            ...
            total_results_count = (
                users_data.get("data", {}).get("metadata", {}).get("totalResultCount", 0)
            )
            elements = users_data.get("data", {}).get("elements", [])
            logger.debug(
                "Found %d elements, total result count: %d", len(elements), total_results_count
            )

            if elements and isinstance(elements, list):
                users, unknown_profiles, limit_data, users_order = parse_default_search_data(
                    elements
                )
            ...
            if mini_profiles and isinstance(mini_profiles, list):
                ...
                if not users:
                    logger.warning(
                        "No found elements with default parser, use fallback users parser"
                    )
                    for item in mini_profiles:
                        item_type = item.get("$type")
                        ...
                        if item_type == "com.linkedin.voyager.dash.search.EntityResultViewModel":
                            ...
                            if not user_public_id and navigation_url:
                                logger.debug(
                                    "Get public_id from navigation URL: %s", navigation_url
                                )
                                try:
                                ...
                                if all(
                                    ...
                                    if image_attributes:
                                        ...
                                        if not vector_image:
                                            ...
                                            if (
                                                profile_picture_urn
                                                and profile_picture_urn in fallback_profiles_images
                                            ):
                                                vector_image = fallback_profiles_images.get(
                                                    profile_picture_urn
                                                )
        ...
        for index, (key, lead) in enumerate(users.items(), start=1):
            ...
            if xstr(lead.get("firstName")) and xstr(lead.get("lastName")):
                if (
                    lead["firstName"].lower().strip() == "linkedin"
                ...
                if xstr(fullname).lower().strip() == "linkedin member":
                    logger.debug(
                        "Reset lead fullname, detected default fullname combination %s", fullname
                    )
                    fullname = None
            ...
            if pictures:
                for image in pictures:
                    if image["width"] == 400:
                        i["picture"] = "%s%s" % (
                            lead["picture"]["rootUrl"],
                            image["fileIdentifyingUrlPathSegment"],
                        )
                        break

            if (
                i.get("profileLink")
                and i.get("profileLinkSN")
                and ((i.get("firstname") and i.get("lastname")) or i.get("fullname"))
            ):
                parsed_users.append(i)

                logger.debug(
                    "Added user to parsed_users list: %s",
                    {
        ...
        if parsed_items:
            for lead in parsed_items:
                ...
                if "entityUrn" in lead:
                    entityUrn = ("").join(
                        re.findall("urn:li:fs_salesProfile:\\((.+?)\\)", lead["entityUrn"])
                    )
                    if entityUrn:
                        i["profileLinkSN"] = "https://www.linkedin.com/sales/people/%s" % entityUrn
                ...
                if "profilePictureDisplayImage" in lead:
                    for image in lead["profilePictureDisplayImage"]["artifacts"]:
                        if image["width"] == 400:
                            i["picture"] = "%s%s" % (
                                lead["profilePictureDisplayImage"]["rootUrl"],
                                image["fileIdentifyingUrlPathSegment"],
                            )
                            break
    ...
    for search_hit in search_hits:
        ...
        try:
            ...
            if search_type == "DEFAULT_SEARCH" and not search_hit_data.get("data"):
                logger.info(
                    "Skip search_hit_data, with %d length, no data key found", len(search_hit_data)
                )
                continue
